...
export async function executeContentHealthCheck(userId: number, text: string) {
  ...
  // 載入學員的 IP 地基和風格資料
  let ipProfile, writingStyle;
  try {
    [ipProfile, writingStyle] = await Promise.all([
      db.getIpProfile(userId),
      db.getUserWritingStyle(userId),
    ]);
    console.log('[executeContentHealthCheck] Loaded IP profile:', !!ipProfile);
    console.log('[executeContentHealthCheck] Loaded writing style:', !!writingStyle);
  } catch (dbError) {
    console.error('[executeContentHealthCheck] DB error:', dbError);
    throw dbError;
  }
  ...
  try {
    ...
    console.log('[executeContentHealthCheck] AI response content type:', typeof content);
    console.log('[executeContentHealthCheck] AI response content (first 500 chars):', typeof content === 'string' ? content.substring(0, 500) : JSON.stringify(content).substring(0, 500));
    aiResult = typeof content === 'string' ? JSON.parse(content) : content;
    console.log('[executeContentHealthCheck] Parsed AI result successfully');
  } catch (e) {
    console.error('[executeContentHealthCheck] JSON parse error:', e);
    console.error('[executeContentHealthCheck] Raw content:', response.choices[0]?.message?.content);
