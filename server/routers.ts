...
const prompt = `你是一位 Threads 內容策略專家。請根據以下創作者資訊，將其專業領域拆解成 5 個具體的「子主題」作為內容支柱。
...
=== 核心原則：X 軸不是教科書目錄，而是「受眾的生活場景」 ===

❗ 嚴禁生成教科書目錄式的子主題！
❌ 錯誤範例：「八字入門」「天干地支」「寵物溝通原理」「基礎理論」
✅ 正確範例：「職場生存法則」「貓咪亂尿尿/日常行為」「免費行銷工具/AI應用」
...
=== 生成要求 ===
請用「${randomAngle}」的角度來拆解，生成 5 個子主題：
1. 必須是受眾會感興趣的具體「生活場景」或「功能需求」
2. 每個子主題都能發展出 10+ 篇貼文
3. 要具體且可執行，不要太籠統
4. 要能觸動受眾的痛點或渴望
5. 絕對不能是教科書目錄或學術名詞
...
=== 檢驗標準 ===
生成後請自我檢查：
1. 這個子主題能與「受眾」交叉產生具體痛點嗎？
2. 這是一般人會搜尋的詞，還是只有專家才懂的詞？
...
{
  "topics": [
    { "name": "子主題名稱", "description": "這個子主題可以寫什麼內容，例如：..." },
    ...
  ]
}
