          ...
          try {
            ...
            if (similarExamples.length > 0) {
              ...
              similarExamples.forEach((ex, i) => {
                ...
                semanticExamplesPrompt += `${i + 1}. [相似度 ${(ex.similarity * 100).toFixed(0)}%] ${preview}...\n`;
              });
        ...
        try {
          ...
          if (similarExamples.length > 0) {
            ...
            similarExamples.forEach((ex, i) => {
              ...
              semanticAnglesPrompt += `${i + 1}. [相似度 ${(ex.similarity * 100).toFixed(0)}%] ${preview}...\n`;
            });
...
❗ 要求：Hook 必須讓這個受眾覺得「這就是在說我」`;
        ...
        try {
          const similarExamples = await findSimilarViralExamples(input.topic, 5);
          if (similarExamples.length > 0) {
            semanticHooksPrompt = `\n=== 語意相似的爆款開頭（從 1,240 篇爆款中找出） ===\n`;
            similarExamples.forEach((ex, i) => {
              const opener = ex.viralExample.opener50 || (ex.viralExample.postText || '').substring(0, 50);
              semanticHooksPrompt += `${i + 1}. [相似度 ${(ex.similarity * 100).toFixed(0)}%] 「${opener}」（${ex.viralExample.likes || 0} 讚）\n`;
            });
