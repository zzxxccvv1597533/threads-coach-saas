...
function generateQuickDiagnosis(
  ...
  // 演算法健檢 — 多層檢查（其餘項目由 client 端結合 dataDriven 組裝）
  type AlgoCheck = {
    level: 'red' | 'yellow' | 'green';
    label: string;
    passed: boolean;
  ...
  const algoChecks: AlgoCheck[] = [];

  // 1. 禁止詞檢查（Red flag）
  const FORBIDDEN_WORDS = ['記住！', '你應該', '一起撐', '溫柔地', '值得被愛'];
  const foundForbidden = FORBIDDEN_WORDS.filter(w => content.includes(w));
  algoChecks.push(foundForbidden.length > 0
    ? {
        level: 'red',
        label: '禁止詞',
        passed: false,
        detail: `發現：${foundForbidden.join('、')} → 建議移除`,
        suggestion: '這些詞會觸發演算法降觸及，請移除或替換',
        chatAction: `請幫我移除或替換以下禁止詞：${foundForbidden.join('、')}`,
      }
    : { level: 'green', label: '禁止詞', passed: true }
  );

  // 2. 結構詞檢查（Yellow flag — AI 感明顯）
  const STRUCTURE_WORDS = ['首先', '其次', '再來', '最後', '總結來說', '綜上所述'];
  const foundStructure = STRUCTURE_WORDS.filter(w => content.includes(w));
  if (foundStructure.length >= 2) {
    algoChecks.push({
      level: 'yellow',
      label: 'AI 結構痕跡',
      passed: false,
      detail: `發現：${foundStructure.join('、')} → AI 寫作特徵`,
      suggestion: '口語化表達不需要「首先、其次」，改用自然的過渡',
      chatAction: `請幫我把 ${foundStructure.join('、')} 改成更口語化的過渡方式，不要用結構詞`,
    });
  }

  // 3. 互動索取行為檢查（Red flag — 降流觸發）
  const ENGAGEMENT_BAIT = /按讚.{0,3}追蹤|追蹤.{0,3}分享|幫我.{0,3}按讚|求.{0,3}分享/;
  if (ENGAGEMENT_BAIT.test(content)) {
    algoChecks.push({
      level: 'red',
      label: '互動索取',
      passed: false,
      detail: '發現「按讚追蹤分享」類語句 → 直接觸發降流',
      suggestion: '不要直接要求按讚追蹤，改用問句引導自然互動',
      chatAction: '請幫我移除所有「按讚追蹤分享」相關語句，改用自然的互動結尾',
    });
  }

  // 4. 外部連結檢查（Red flag — 降流觸發）
  const LINK_PATTERN = /https?:\/\/|bit\.ly|linktr\.ee|line\.me/i;
  if (LINK_PATTERN.test(content)) {
    algoChecks.push({
      level: 'red',
      label: '外部連結',
      passed: false,
      detail: '偵測到外部連結 → Threads 會直接降觸及',
      suggestion: '移除所有連結，改用「連結在個人簡介」',
      chatAction: '請幫我移除貼文中的所有外部連結',
    });
  }

  // 鐵律 #1：開場用「我」不用「你」
  const firstNonEmptyLine = content.split('\n').find(l => l.trim().length > 0) || '';
  if (/^(你|各位|大家|親愛的)/.test(firstNonEmptyLine.trim())) {
    algoChecks.push({
      level: 'yellow' as const,
      label: '開場視角',
      passed: false,
      detail: '開頭用「你」開場，建議改用「我」（觀察者視角）',
      suggestion: '把「你是不是也...」改成「我發現...」或「我遇過...」',
      chatAction: '請把開頭改成用「我」的視角，例如「我發現...」「我遇過一個...」',
    });
  }

  // 鐵律 #4：結尾用反問不用總結
  const nonEmptyLines = content.split('\n').filter(l => l.trim().length > 0);
  const lastThreeLines = nonEmptyLines.slice(-3).join('');
  if (/總之|總結|記住|所以說|歸納/.test(lastThreeLines) && !/？/.test(lastThreeLines)) {
    algoChecks.push({
      level: 'yellow' as const,
      label: '結尾方式',
      passed: false,
      detail: '結尾像總結語，建議改成反問引導互動',
      suggestion: '把「所以記住...」改成「你呢？」「你也是這樣嗎？」',
      chatAction: '請把結尾改成反問句，例如「你也遇過這種情況嗎？」',
    });
  }

  // 鐵律 #5：句子長度檢查（超過 40 字的行太多）
  ...
  if (longLines.length >= 3) {
    algoChecks.push({
      ...
      label: '句子長度',
      passed: false,
      detail: `有 ${longLines.length} 行超過 40 字，建議一行一句`,
      suggestion: '每句 15-20 字最佳，超過就斷行',
      chatAction: '請幫我把太長的句子拆成短句，每句 15-20 字，一行一句',
    });
  }

  const hasFailure = algoChecks.some(c => !c.passed);
  const hasRed = algoChecks.some(c => c.level === 'red' && !c.passed);
  const algorithmChecks = {
    checks: algoChecks,
    overallLevel: hasRed ? 'red' as const : hasFailure ? 'yellow' as const : 'green' as const,
  };

  return { strengths, improvements, score, algorithmChecks };
}
    ...
    generateDraft: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const buildIpContext = () => {
          ...
          // 完整導入模式（full_professional）：以第���人稱身份注入 IP 地基
...
1. 描述一個你觀察到的現象或困惑
2. 把你的疑惑或好奇丟出來
3. 用「你也是這樣嗎？」或「你會怎麼選？」結尾
⚠️ 不要自己回答問題，讓讀者回答。你的角色是拋出好問題，不是給答案
...
【投票型貼文結構】
1. 用 1-2 句話設定一個有爭議的情境
2. 丟出二選一（「A 還是 B？」）
3. 不要表態，讓留言區自己炸
⚠️ 二選一的選項要都有道理，不能有明顯正確答案
...
1. 拋出大家的普遍認知（「大部分人以為...」）
2. 用你的經歷反駁（「但我看到的是...」）
3. 給出你的判斷標準（「我怎麼判斷？我會看...」）
4. 留下開放討論（「你們覺得呢？」）
⚠️ 觀點要有你的專業底氣支撐，不能只是個人感覺
...
1. 跟上面的貼文用不同的開頭方式和切入角度（避免重複）
2. 但要保持同一個人的說話風格（讓讀者一看就知道是你寫的）
3. 如果上面有用到括號 OS、反問結尾、具體數字 — 這篇也要有（這是你的風格標記）\n`;
        }
...
不能只說受眾的痛點，要用「你的經歷」回應痛點。

❌ 錯誤：「很多人學完技術不知道怎麼接客」（只有痛點，任何人都能寫）
✅ 正確：「我合作過一個老師，技術好到客人都哭了，但一個月只接 2 個客」（用你的真實經歷回應痛點）

公式：受眾痛點 + 你的親身觀察 + 你的專業判斷 = 有血有肉的內容
...
寫每一段的時候，問自己：
- 這段有沒有展現「專業底氣」？（你看過的案例、你的判斷標準、你的數據）
- 這段有沒有帶到「走過的路」？（你曾經也犯過的錯、你的轉變、你的掙扎）
- 這段有沒有體現「跟別人不同的觀點」？（你的核心信念、你的獨特見解）

整篇文章至少要涵蓋其中兩個角度。

範例（好）：「我合作過 30 個老師，發現最會賺的那個技術反而最普通」→ 專業底氣 + 獨特觀點
範例（壞）：「很多人覺得技術很重要」→ 任何人都能說的話

⚠️ 如果某一段拿掉作者的名字，讀起來像 ChatGPT 寫的 → 那段要重寫。用具體的數字、真實的場景、你親眼看到的事來取代。
...
1. 開場永遠用「我」不用「你」— 第一句建立觀察者視角
2. 數字要具體到不像編的 — 「143 個」不是「很多人」
3. 每篇要有一句可截圖分享的金句 — 獨立出來也能成立
4. 結尾用反問不用總結 — 「你呢？」不是「所以記住」
5. 一行一句不堆長段落 — 每 2-3 行空一行
6. 括號 OS 增加人味 — "(好吧也有可能是我想太多)"
7. 不說「你應該」說「我發現」— 觀察 > 說教
...
請直接輸出精簡後的文章，不要有任何前置說明。記住：絕對不能超過 ${wordLimit.max} 字！` 
    ...
    // ==================== Smart Generate — 策略判斷（4 步流程用） ====================
    // 根據 topic + material + IP 地基，自動決定 contentType、切角、Hook 策略
    // Client 拿到結果後再呼叫 generateDraft
    smartStrategy: protectedProcedure
      .input(z.object({
        topic: z.string(),
        material: z.string(), // 已組合的素材文字
      }))
      .mutation(async ({ ctx, input }) => {
        const profile = await db.getIpProfileByUserId(ctx.user.id);
        const audiences = await db.getAudienceSegmentsByUserId(ctx.user.id);
        ...
        if (profile?.personaExpertise) ipSummary += `專業權威：${profile.personaExpertise}\n`;
        ...
        let audienceContext = '';
        if (audiences && audiences.length > 0) {
          audienceContext = audiences.map((a, i) =>
            `${i}. ${a.segmentName}：痛點「${a.painPoint || '未設定'}」，渴望「${a.desiredOutcome || '未設定'}」`
          ).join('\n');
        }

        const strategyResponse = await invokeLLM({
          messages: [
            { role: "system", content: `你是 Threads 貼文策略專家。根據作者的主題、素材和個人定位，判斷最佳的貼文策略。

## 可選內容類型
- story（故事型）：用真實故事建立信任，適合有具體案例的素材
- knowledge（知識型）：分享專業知識建立權威，適合有方法論的素材
- viewpoint（觀點型）：表達獨特觀點引發討論，適合有反直覺觀點的素材
- question（提問型）：拋出問題引導留言，適合想衝互動的時候
- casual（日常閃文）：輕鬆分享日常，適合素材簡短或情緒性的時候
- contrast（反差型）：製造反差衝擊，適合有前後對比的案例
- dialogue（對話型）：用對話呈現觀點，適合有對話場景的素材
- poll（投票型）：二選一引爆留言
- summary（懶人包）：整理知識方便分享
- quote（金句型）：一句話點醒人

## 可選切角方式
- daily_scenario（日常場景）：從生活中的具體場景切入
- common_mistakes（常見錯誤）：指出受眾的盲區
- perspective_flip（觀點翻轉）：反轉常規思維製造衝擊
- trending_leverage（時事借力）：借熱門話題切入

## 判斷原則
1. 素材中有具體故事/案例 → 優先 story 或 contrast
...
請判斷最佳策略：` }
          ...
          model: getModelForFeature('brainstorm'),
          response_format: {
            type: "json_schema",
            json_schema: {
              ...
              schema: {
                type: "object",
                properties: {
                  contentType: { type: "string", description: "選擇的內容類型 ID" },
                  angle: { type: "string", description: "選擇的切角方式 ID" },
                  hookStrategy: { type: "string", enum: ["mirror", "contrast", "solution"], description: "Hook 心理策略" },
                  targetAudienceIndex: { type: "number", description: "最適合的受眾索引（0-based），-1 表示通用" },
                  reasoning: { type: "string", description: "一句話解釋為什麼這樣選" },
                },
                required: ["contentType", "angle", "hookStrategy", "targetAudienceIndex", "reasoning"],
                additionalProperties: false,
              },
            },
          },
        ...
        let strategy = {
          contentType: 'story',
          angle: 'daily_scenario',
          hookStrategy: 'mirror' as string,
          targetAudienceId: undefined as number | undefined,
          reasoning: '',
        };

        try {
          const parsed = JSON.parse(typeof strategyResponse === 'string' ? strategyResponse : (strategyResponse as any).content || '{}');
          strategy.contentType = parsed.contentType || 'story';
          strategy.angle = parsed.angle || 'daily_scenario';
          strategy.hookStrategy = parsed.hookStrategy || 'mirror';
          strategy.reasoning = parsed.reasoning || '';

          if (parsed.targetAudienceIndex >= 0 && audiences && audiences[parsed.targetAudienceIndex]) {
            strategy.targetAudienceId = audiences[parsed.targetAudienceIndex].id;
          }
        } catch (e) {
          console.warn('[smartStrategy] Strategy parse failed, using defaults:', e);
        }

        // 驗證 contentType
        const validTypes = ['story', 'knowledge', 'viewpoint', 'question', 'casual', 'contrast', 'dialogue', 'poll', 'summary', 'quote', 'diagnosis'];
        if (!validTypes.includes(strategy.contentType)) {
          strategy.contentType = 'story';
        }

        const angleLabels: Record<string, string> = {
          'daily_scenario': '日常場景',
          'common_mistakes': '常見錯誤',
          'perspective_flip': '觀點翻轉',
          'trending_leverage': '時事借力',
        };

        return {
          contentType: strategy.contentType,
          angle: strategy.angle,
          angleLabel: angleLabels[strategy.angle] || strategy.angle,
...
- 可以問如何連結到產品/服務`,
        ...
        const typeInstruction = contentType
          ? `貼文類型：${contentType}\n${typeContext[contentType] || typeContext.story}`
          : `貼文類型：尚未決定（請生成能廣泛收集素材的問題）
